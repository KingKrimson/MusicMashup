...
(album, track, artist) is RSS 2.0 format. After processing, the file is simply
...
if (!$db_server = mysqli_connect($db_hostname, $db_username, $db_password, $db_database)) {
...
} else { //get the most recently favourited items. Sort by most recent, so we
         //know that they're already in some kind of order.
    $artistquery = "SELECT artistdatetimefavourited, username, userid, artistname, artistid FROM favouriteartists " .
            "NATURAL JOIN user NATURAL JOIN artist ORDER BY artistdatetimefavourited ASC LIMIT 20";
    $albumquery = "SELECT albumdatetimefavourited, username, userid, albumname, albumid FROM favouritealbums " .
            "NATURAL JOIN user NATURAL JOIN album ORDER BY albumdatetimefavourited ASC LIMIT 20";
    $trackquery = "SELECT trackdatetimefavourited, username, userid, trackname, trackid FROM favouritetracks " .
            "NATURAL JOIN user NATURAL JOIN track ORDER BY trackdatetimefavourited ASC LIMIT 20";
    ...
    for ($i = 0; $i < 20; ++$i) {
        ...
        echo "            <description>$username favourited the $type $itemname at $time</description>\n";
...
<rss version="2.0"> 
    <channel> 
        <title>Alveston_Weather-23:57:36</title> 
        <link>http://www.alvestonweather.co.uk</link> 
        <item> 
            <title>Weather at 23:57</title> 
            <description> Night_time/Dry . Wind 9.4 knots from 41  degrees. Temperature 3.4 &#0176;C. </description> 
        </item> 
    </channel> 
</rss>
*/
